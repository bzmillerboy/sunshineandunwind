...
const fetchDocuments = () =>
  client.fetch(
    `*[_type == "equipmentCategory" && exclude != true && body != null && description == null  ] {_id, title, body, description}`
  );

const buildPatches = docs =>
  docs.map(doc => {
    console.log("doc:", doc);
    const heroBodyFromDoc = deepSearchByKey(doc.body, "_type", "hero") || null;
    console.log("heroBodyFromDoc:", heroBodyFromDoc);
    const heroBodyFromDocValue = heroBodyFromDoc[0].body || "none";
    console.log("heroBodyFromDocValue:", heroBodyFromDocValue);

    return {
      id: doc._id,
      patch: {
        set: { description: heroBodyFromDocValue },
        // unset: ['name'],
