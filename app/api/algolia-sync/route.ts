...
const algoliaSyncSecret = process.env.SANITY_ALGOLIA_WEBHOOK_SECRET
...
export async function POST(req: NextRequest) {
  ...
  const { body, isValidSignature } = await parseBody(req, algoliaSyncSecret)
  if (algoliaSyncSecret && !isValidSignature) {
    console.log('algolia-sync isValidSignature', isValidSignature)
    return new Response('Invalid signature', { status: 401 })
  }

  // const reqBody = await req.json()
  const webhookBody = body as unknown as WebhookBody
  ...
  return sanityAlgolia
    .webhookSync(sanity, webhookBody)
    .then(() => {
