...
const formSchema = z.object({
  ...
  // type must be either 1 or 2 strings, if  not display an error message.
  type: z
    .string()
    .nullish()
    .refine((val) => val !== null && (val === '1' || val === '2'), {
      message:
        'Please select a type of "Equipment Financing" or "Rentals, Parts, & Service".',
    }),
...
export default function RequestFinancingFormStep1({
  ...
  async function onSubmit(data: z.infer<typeof formSchema>, event: any) {
    ...
    await fetch(`/function/financing-application`, {
      method: 'POST',
      headers: {
      ...
      >
        ...
        <div className="col-span-2">
          <FormField
            ...
            name="type"
            render={({ field }) => (
              <FormItem className="grid grid-cols-1 text-xs md:grid-cols-2 gap-2 space-y-0">
                <FormControl>
                  <div>
                    <input
                      {...register('type')}
